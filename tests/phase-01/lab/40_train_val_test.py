import copy
import os
import random
import numpy as np
import torch
import torch.nn as nn

from mini_kuzai_deep import MiniKuzaiDeep


# ==================================================
# Determinism
# ==================================================

os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"

SEED = 42

random.seed(SEED)
np.random.seed(SEED)
torch.manual_seed(SEED)

if torch.cuda.is_available():
    torch.cuda.manual_seed_all(SEED)

torch.use_deterministic_algorithms(True)

device = torch.device(
    "cuda" if torch.cuda.is_available() else "cpu"
)

EOS = "<eos>"


# ==================================================
# TRAIN
# Only this set modifies the weights.
# ==================================================

train_lines = [
    "mini kuzai runs on linux",
    "mini kuzai learns from data",
    "mini kuzai can generate text",
    "mini kuzai is a language model",
    "a language model learns from data",
    "a language model can generate text",
    "linux runs a model",
    "data helps a model learn",
    "mini kuzai uses a model",
    "a model uses data",
    "a model uses linux",
    "text uses data",

    "linux is a system",
    "mini kuzai uses linux",
    "a model runs using linux",
    "linux can run a model",
    "data helps learning",
    "a model can learn from data",
    "mini kuzai learns using data",
    "data is used by a model",
]


# ==================================================
# VALIDATION
# Used ONLY for early stopping.
# ==================================================

validation_lines = [
    "a language model uses data",
    "linux can generate text",
    "mini kuzai can learn from data",
]


# ==================================================
# TEST
# Never used for training or early stopping.
# ==================================================

tests = [
    ("a language model runs on", "linux"),
    ("a model runs on", "linux"),
    ("a model learns from", "data"),
]


# ==================================================
# Vocabulary from TRAIN only
# ==================================================

tokens = []

for line in train_lines:
    tokens.extend(line.split())
    tokens.append(EOS)

vocabulary = sorted(set(tokens))

token_to_id = {
    token: i
    for i, token in enumerate(vocabulary)
}

id_to_token = {
    i: token
    for token, i in token_to_id.items()
}


# ==================================================
# Safety checks
# ==================================================

def prefix_seen(prefix, dataset):

    prefix_words = prefix.split()

    for line in dataset:

        words = line.split()
        n = len(prefix_words)

        for i in range(len(words) - n + 1):

            if words[i:i+n] == prefix_words:
                return True

    return False


validation_unknown = sorted({
    word
    for line in validation_lines
    for word in line.split()
    if word not in token_to_id
})

test_unknown = sorted({
    word
    for prompt, expected in tests
    for word in prompt.split() + [expected]
    if word not in token_to_id
})

validation_duplicates = sorted(
    set(train_lines) & set(validation_lines)
)

test_prefix_leakage = [
    prompt
    for prompt, _ in tests
    if prefix_seen(prompt, train_lines)
]

if validation_unknown:
    raise RuntimeError(
        f"Unknown validation tokens: {validation_unknown}"
    )

if test_unknown:
    raise RuntimeError(
        f"Unknown test tokens: {test_unknown}"
    )

if validation_duplicates:
    raise RuntimeError(
        f"Validation leakage: {validation_duplicates}"
    )

if test_prefix_leakage:
    raise RuntimeError(
        f"Test prefix leakage: {test_prefix_leakage}"
    )


# ==================================================
# Model
# ==================================================

model = MiniKuzaiDeep(
    vocab_size=len(vocabulary),
    embedding_dim=8,
    hidden_dim=32,
    num_heads=2,
    num_layers=2,
    max_sequence_length=32
).to(device)

criterion = nn.CrossEntropyLoss()

optimizer = torch.optim.AdamW(
    model.parameters(),
    lr=0.01
)


# ==================================================
# Dataset evaluation
# ==================================================

def evaluate_lines(dataset):

    model.eval()

    total = 0.0

    with torch.no_grad():

        for line in dataset:

            words = line.split() + [EOS]

            ids = torch.tensor(
                [token_to_id[w] for w in words],
                dtype=torch.long,
                device=device
            )

            logits = model(ids[:-1])

            loss = criterion(
                logits,
                ids[1:]
            )

            total += loss.item()

    return total / len(dataset)


# ==================================================
# Training + early stopping
# ==================================================

max_epochs = 400
patience = 30

best_validation_loss = float("inf")
best_epoch = 0
best_state = None

without_improvement = 0


for epoch in range(1, max_epochs + 1):

    model.train()

    for line in train_lines:

        words = line.split() + [EOS]

        ids = torch.tensor(
            [token_to_id[w] for w in words],
            dtype=torch.long,
            device=device
        )

        inputs = ids[:-1]
        targets = ids[1:]

        optimizer.zero_grad()

        logits = model(inputs)

        loss = criterion(
            logits,
            targets
        )

        loss.backward()
        optimizer.step()


    val_loss = evaluate_lines(
        validation_lines
    )


    if val_loss < best_validation_loss:

        best_validation_loss = val_loss
        best_epoch = epoch

        best_state = copy.deepcopy(
            model.state_dict()
        )

        without_improvement = 0

    else:

        without_improvement += 1


    if without_improvement >= patience:
        stopped_epoch = epoch
        break

else:
    stopped_epoch = max_epochs


# ==================================================
# Restore best VALIDATION model
# ==================================================

model.load_state_dict(best_state)
model.eval()


# ==================================================
# Independent TEST
# ==================================================

def test_prediction(prompt, expected):

    ids = torch.tensor(
        [token_to_id[w] for w in prompt.split()],
        dtype=torch.long,
        device=device
    )

    with torch.no_grad():

        probabilities = torch.softmax(
            model(ids)[-1],
            dim=-1
        )

    probs, ids_sorted = torch.sort(
        probabilities,
        descending=True
    )

    expected_id = token_to_id[expected]

    expected_rank = (
        (ids_sorted == expected_id)
        .nonzero(as_tuple=True)[0]
        .item()
        + 1
    )

    predicted = id_to_token[
        ids_sorted[0].item()
    ]

    expected_probability = probabilities[
        expected_id
    ].item()

    return (
        predicted,
        expected_rank,
        expected_probability
    )


# ==================================================
# Save
# ==================================================

torch.save(
    {
        "model_state_dict": best_state,
        "vocabulary": vocabulary,
        "token_to_id": token_to_id,
        "id_to_token": id_to_token,
        "embedding_dim": 8,
        "hidden_dim": 32,
        "num_heads": 2,
        "num_layers": 2,
        "max_sequence_length": 32,
        "eos_token": EOS,
        "best_epoch": best_epoch,
        "best_validation_loss": best_validation_loss,
    },
    "mini-kuzai-tvt.pt"
)


# ==================================================
# Results
# ==================================================

print("##### SECTION RESULTS FOR ASSISTANT ######")

print()
print("===== TRAIN / VALIDATION / TEST =====")

print("Device                :", device)

print("Training sentences    :", len(train_lines))
print("Validation sentences  :", len(validation_lines))
print("Independent test cases:", len(tests))

print()
print("Vocabulary size       :", len(vocabulary))
print("Model parameters      :", sum(
    p.numel()
    for p in model.parameters()
))

print()
print("Unknown validation    :", len(validation_unknown))
print("Unknown test          :", len(test_unknown))
print("Validation duplicates :", len(validation_duplicates))
print("Test prefix leakage   :", len(test_prefix_leakage))

print()
print("===== EARLY STOPPING =====")

print("Best epoch            :", best_epoch)
print(
    "Best validation loss :",
    f"{best_validation_loss:.6f}"
)

print("Stopped epoch         :", stopped_epoch)

print()
print("===== INDEPENDENT TEST =====")

for prompt, expected in tests:

    predicted, rank, probability = (
        test_prediction(
            prompt,
            expected
        )
    )

    print()
    print("PROMPT   :", prompt)
    print("EXPECTED :", expected)
    print("PREDICTED:", predicted)
    print("RANK     :", rank)

    print(
        "EXPECTED PROBABILITY:",
        f"{probability * 100:.2f} %"
    )

    print(
        "TOP-1 CORRECT:",
        predicted == expected
    )

print()
print("Checkpoint: mini-kuzai-tvt.pt")
